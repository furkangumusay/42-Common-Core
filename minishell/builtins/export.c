...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/06/24 06:53:18 by fgumusay          #+#    #+#             */
...
static int	is_env_exist(t_env *env, char *key)
{
	while (env)
	{
		if ((ft_strlen(env->key) == ft_strlen(key)) && ft_strcmp(env->key,
				key) == 0)
			return (TRUE);
		env = env->next;
	}
...
static int	input_of_export(t_mshell *shell, t_command *command)
{
	...
	while (command->args[++vars.i])
	{
		...
		else if (ft_strncmp(find_env(shell, vars.name), vars.content,
