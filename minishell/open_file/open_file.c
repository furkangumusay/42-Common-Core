...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/06/24 06:55:23 by fgumusay          #+#    #+#             */
...
{
	if ((*lexer)->type == T_REDIR_APPEND && (*lexer)->next != NULL)
	{
		(*lexer) = (*lexer)->next;
		expander(shell, (*lexer));
		unquote_lexer_content(shell, (*lexer));
		open_output_file((*lexer)->content, T_REDIR_APPEND, command);
	}
	else if ((*lexer)->type == T_REDIR_IN && (*lexer)->next != NULL)
	{
		(*lexer) = (*lexer)->next;
		unquote_lexer_content(shell, (*lexer));
		open_input_file((*lexer)->content, command);
	}
	else if ((*lexer)->type == T_REDIR_OUT && (*lexer)->next != NULL)
	{
		(*lexer) = (*lexer)->next;
		expander(shell, (*lexer));
		unquote_lexer_content(shell, (*lexer));
		open_output_file((*lexer)->content, T_REDIR_OUT, command);
	}
}
